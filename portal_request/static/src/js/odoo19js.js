...
$(document).ready(function () {
    ...
    let saveChangedFieldsValues = async function(thiss){
    ...
    async function TriggerProductField(lastRow_count){
        // PRODUCTSEARCH
        let oldValue = $(`select[special_id='${lastRow_count}']`).val()
        await $(`select[special_id='${lastRow_count}']`).select2({
            ajax: {
    ...
    async function searchStockLocation(element, source, classes=''){
        // find the input field
        const elm = $(`select[name=source_location_id]`);
    ...
    async function triggerVendor(){
        // find the input field
        const elm = $(`select[name=vendor_id_form]`);
    ...
    async function triggerinputFollowers() {
        await $('#inputFollowers').select2({
         ajax: {
    ...
    async function searchStockLocation2(element="destination_location_id", source="destination", classes=''){
        // find the input field
        const elm = $(`select[name=destination_location_id]`);
        let oldValue = elm.val(); // OGIDI
        let oldId = elm.attr('id'); 
        await elm.select2({
            ajax: {
    ...
    publicWidget.registry.PortalRequestFormWidgets = publicWidget.Widget.extend({
        ...
        willStart: function(){
            ...
            return this._super.apply(this, arguments).then(async function(){
                await loadJS('/portal_request/static/src/js/core/select2.min.js');
                await loadCSS('/portal_request/static/src/css/select2.min.css');
                await loadCSS('/portal_request/static/src/css/portal_css.css');
                await loadJS("https://code.jquery.com/ui/1.13.2/jquery-ui.min.js");
                await loadCSS("https://code.jquery.com/ui/1.13.2/themes/base/jquery-ui.css");
        ...
        events: { 
            ...
            'click .editbtn': function(ev){
                ...
                triggerVendor();
                triggerinputFollowers();
            }, 

            'click #save': async function(ev){
                // hide save btn 
                // hide discard button
                ...
                let leave_reliever_ids = $("#leave_reliever_ids")
                let description = $("#description")
                let source_location_id = $("select[name=source_location_id]")
                let dest_location_id = $("select[name=destination_location_id]")
                let vendor_id_form = $("select[name=vendor_id_form]")
            ...
            'change select[name=leave_type_id]': async function(ev){
                ...
                if(staff_num !== '' && leave_id !== ''){  
                    var self = this;
                    await rpc(`/get/leave-allocation`, ///${leave_id}/${staff_num}`,
                        {
			...
			'change #leave_reliever_ids': async function(ev){
            // 'blur input[name=leave_reliever]': function(ev){
                let leave_reliever = $('#leave_reliever_ids');
                let start_date = $('#leave_start_datex');
                if (!start_date.val() && leave_reliever.val() !== null){
					...
					if (leave_reliever.val() !== null){
					    await rpc(
                            `/check-employee-still-onleave`,
							{
            ...
            'click .supervisor_comment_button': async function(ev){
            ...
            'click .refuse_comment_button': async function(ev){
                let targetElement = $(ev.target).attr('id');
                let $btn = $('.refuse_comment_button');
                let $btnHtml = $btn.html()
                $btn.attr('disabled', 'disabled');
            ...
            'click .refuse_request_btn': async function(ev){
                let targetElement = $(ev.target).attr('id');
            ...
            'click .resend_request': async function(ev){
                let targetElementid = $('.record_id').attr('id');
            ...
            'click .approve_request': async function(ev){
                let targetElementId = $('.record_id').attr('id');
