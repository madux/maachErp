...
class AccountReport(models.AbstractModel):
    ...
    def _set_context(self, options):
        ...
        if options.get('branch'):
            ctx['branch'] = self.env['eha.branch'].browse([int(acc) for acc in options['branch']])
    ...
    def get_report_informations(self, options):
        ...
        """the method no longer exists in odoo v13"""
        # self._apply_date_filter(options)

        searchview_dict = {'options': options, 'context': self.env.context}
        # Check if report needs analytic
        searchview_dict['branch'] = [(t.id, t.name) for t in
                                                           self.env['eha.branch'].search([])] or False
        # options['selected_branch_names'] = [self.env['eha.branch'].browse(int(tag)).name for tag in
        #                                           options['branch']]
        options['selected_branch_names'] = [self.env['eha.branch'].browse([int(tag.id)]).name for tag in
                                                  self.env['eha.branch'].search([])]
