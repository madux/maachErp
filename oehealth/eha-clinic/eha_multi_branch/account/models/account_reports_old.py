...
class AccountReportExtension(models.AbstractModel):
    ...
    def _build_options(self, previous_options=None):
        ...
        branches_read = self.env['eha.branch'].search([('company_id', 'in', self.env.user.company_ids.ids or [self.env.user.company_id.id])], order="company_id, name")
    ...
    def _get_branches(self):
        branches_read = self.env['eha.branch'].search([], order="id, name")
        branches = []
        previous_company = False
        for b in branches_read:
            if b.company_id != previous_company:
