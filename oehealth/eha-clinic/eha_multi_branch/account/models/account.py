...
class AccountReconcileModel(models.Model):
    ...
    branch_id = fields.Many2one(
        'eha.branch', 'Branch', default=lambda self: self.env.user.branch_id.id)
...
class AccountInvoice(models.Model):
    ...
    branch_id = fields.Many2one('eha.branch', 'Branch', default=lambda self: self.env.user.branch_id.id, states={
                                'draft': [('readonly', False)]})
    bill_to = fields.Many2one('res.partner')

    # ---------------------------------------
    # Process healthmate invoices
    # ---------------------------------------
    def _trigger_cloud_healthmate_invoice_function(self):
        for rec in self:
            user = self.env["res.users"].search([
                ('partner_id', '=', rec.partner_id.id)])
            email = rec.partner_id.email or user.email
            if email:
                base_url = self.env['ir.config_parameter'].sudo(
                ).get_param('web.base.url')
                invoiceurl = f"{base_url}/web#id={self.id}"
                invoice_payload = {
                    "id": rec.id,
                    "invoice_code": re.sub("/", "-", rec.name),
                    "reference_number": rec.ref,
                    "invoice_date": f"{rec.invoice_date:%Y-%m-%d}",
                    "invoice_url": invoiceurl,
                    "due_date": f"{rec.invoice_date_due:%Y-%m-%d}",
                    "source": rec.invoice_origin or None,
                    "total": rec.amount_total,
                    "amount_due": rec.amount_residual,
                    "bank": "", # rec.partner_bank_id.bank_id.name,
                    "account_number": "", # rec.partner_bank_id.acc_number,
                    'lines': [
                        {
                            'product': sl.product_id.name,
                            'quantity': sl.quantity,
                            'unit_price': sl.price_unit,
                            'amount': sl.price_subtotal,
                        } for sl in rec.invoice_line_ids
                    ]
                }
                api_key = self.env['ir.config_parameter'].sudo().get_param(
                    'healthmate_cloud_function_api_key')
                # url = f"https://europe-west1-clinics-companion-app-dev.cloudfunctions.net/webApi/api/v1/invoice/{email}"
                healthmate_url = self.env['ir.config_parameter'].sudo().get_param("process_healthmate_invoice_url")
                url = f"{healthmate_url}{email}"
                headers = {
                    "X-API-KEY": api_key,
                    "Content-Type": "Application/json",
                }
                _logger.info(f"----- My payload ------ {invoice_payload}")
                res = requests.post(
                    url=url, headers=headers, data=json.dumps(invoice_payload))
                # _logger.info(f"&&&&& response from api call {res.json()}")

    def action_post(self):
        for post in self:
            res = super(AccountInvoice, self).action_post()
            try:
                post._trigger_cloud_healthmate_invoice_function()
...
class AccountPartialReconcile(models.Model):
    ...
    branch_id = fields.Many2one('eha.branch', related='debit_move_id.branch_id', string='Branch',
...
class AccountJournal(models.Model):
    ...
    branch_id = fields.Many2one(
        'eha.branch', 'Branch', default=lambda self: self.env['eha.branch']._branch_default_get(), required=False)


class AccountAnalyticLine(models.Model):
    _inherit = "account.analytic.line"
    branch_id = fields.Many2one(
        'eha.branch', 'Branch', default=lambda self: self.env.user.partner_id.branch_id.id, required=False)


# class AccountAnalyticDefault(models.Model):
#     _inherit = "account.analytic.default"
#     branch_id = fields.Many2one(
#         'eha.branch', 'Branch', default=lambda self: self.env.user.partner_id.branch_id.id)
...
#     branch_id = fields.Many2one('eha.branch', string='Branch', default=lambda self: self.env.user.partner_id.branch_id.id)
...
#
#     branch_id = fields.Many2one(
#         'eha.branch', 'Branch', default=_get_invoice_refund_default_branch, required=False)
#
...
class AccountBankStatementLine(models.Model):
    ...
    branch_id = fields.Many2one(
        'eha.branch', 'Branch', related='statement_id.branch_id')
