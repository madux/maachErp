        ...
        if self:
            ...
            for res_model, res_id, create_uid, public, res_field in self._cr.fetchall():
                ...
                if not self.env.is_system() and res_field:
                    '''Below commented code is the latest code but our current requirement enables user to view documents for now
                    This affects all records including sign module if it computes any write operation and the current user is not the
                    creator or not a user with admin settings
                    '''
