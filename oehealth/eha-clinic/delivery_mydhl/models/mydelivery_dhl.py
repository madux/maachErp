...
from .mydhl_request import MyDHLProvider

from odoo import models, fields, _

class ProviderMydhl(models.Model):
    _inherit = 'delivery.carrier'

    delivery_type = fields.Selection(selection_add=[('mydhl', "My DHL")], ondelete={'mydhl': lambda recs: recs.write({
            'delivery_type': 'fixed', 'fixed_price': 0})})
    dhl_username = fields.Char(string="MyDHL Username", groups="base.group_system")
    dhl_password = fields.Char(string="MyDHL Password", groups="base.group_system")
    dhl_account_number = fields.Char(string="MyDHL Account Number", groups="base.group_system")
    ...
    dhl_default_packaging_id = fields.Many2one('stock.package.type', string='DHL Default Packaging Type')
    ...
    def mydhl_rate_shipment(self, order):
        srm = MyDHLProvider(self.prod_environment, self.log_xml)
    ...
    def mydhl_send_shipping(self, pickings):
        res = []
    ...
    def mydhl_get_tracking_link(self, picking):
        '''
        Return DHL tracking URL   
        'http://www.dhl.com/en/express/tracking.html?AWB=%s' % picking.carrier_tracking_ref
        '''
        return "https://www.logistics.dhl/global-en/home/tracking.html?tracking-id=%s" % picking.carrier_tracking_ref

    def mydhl_cancel_shipment(self, picking):
    ...
    def _mydhl_convert_weight(self, weight, unit):
