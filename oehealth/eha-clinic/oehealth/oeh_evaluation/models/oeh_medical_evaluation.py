...
class OeHealthPatientEvaluation(models.Model):
    ...
    name = fields.Char(string='Evaluation #', size=64, readonly=True, required=False, default=lambda *a: '/')
    patient = fields.Many2one('oeh.medical.patient', string='Patient', help="Patient Name", required=False, index=True)
    doctor = fields.Many2one('oeh.medical.physician', string='Physician', help="Current primary care / family doctor", domain=[('is_pharmacist','=',False)], required=False,
    ...
    evaluation_type = fields.Selection(EVALUATION_TYPE, string='Evaluation Type', required=False, index=True, default=lambda *a: 'Pre-arraganged Appointment',
                                       ondelete={'Ambulatory': lambda recs: recs.write({
            'evaluation_type': 'fixed', 'fixed_price': 0})})
