...
class OeHealthRecreationalDrug(models.Model):
    ...
    name = fields.Char(string='Drug Name', size=128, required=False)
    street_name = fields.Char(string='Street Names', size=256, help="Common name of the drug in street jargon")
    toxicity = fields.Selection(TOXICITY, string='Toxicity', required=False)
    addiction_level = fields.Selection(ADDICTION_LEVEL, string='Addiction Level', required=False)
    legal_status = fields.Selection(LEGAL, string='Legal Status', required=False)
    category = fields.Selection(DRUG_CATEGORY, 'Category', required=False)
