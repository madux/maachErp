...
class OeHealthFamily(models.Model):
    ...
    name = fields.Char(size=256, string='Name', required=False, help='Family Member Name')
    ...
    patient_id = fields.Many2one('oeh.medical.patient', 'Patient', required=False, ondelete='cascade', index=True)
...
class OeHealthPatient(models.Model):
    ...
    def _admission_count(self):
        oe_admission = self.env['oeh.medical.admission']
        for adm in self:
            domain = [('patient_id', '=', adm.id)]
        ...
        def compute_age_from_dates (patient_dob,patient_deceased,patient_dod):
            ...
            if (patient_dob):
                ...
                if patient_deceased:
                    ...
                    delta = dod - dob
                    deceased = " (deceased)"
                    years_months_days = str(int(delta.days // 365.2425)) + " years " + str(
                        int(math.floor(delta.days % 365.2425))) + " days" + deceased
                else:
                    delta = now - dob
                    years_months_days = str(int(delta.days // 365.2425)) + " years " + str(
                        int(math.floor(delta.days % 365.2425))) + " days"
            else:
    ...
    partner_id = fields.Many2one('res.partner', string='Related Partner', required=False, index=True, ondelete='cascade', help='Partner-related data of the patient')
    ...
    identification_code = fields.Char(string='Patient ID', size=256, help='Patient Identifier provided by the Health Center')
...
class OeHealthPhysicianSpeciality(models.Model):
    ...
    name = fields.Char(string='Description', size=128, help="ie, Addiction Psychiatry", required=False)
...
class OeHealthPhysicianDegree(models.Model):
    ...
    name = fields.Char(string='Degree', size=128, required=False)
...
class OeHealthAppointment(models.Model):
    ...
    patient = fields.Many2one('oeh.medical.patient', string='Patient', help="Patient Name", required=False, readonly=True, states={'Scheduled': [('readonly', False)]})
    doctor = fields.Many2one('oeh.medical.physician', string='Physician', help="Current primary care / family doctor", domain=[('is_pharmacist','=',False)], required=False, readonly=True,states={'Scheduled': [('readonly', False)]}, default=_get_physician)
    appointment_date = fields.Datetime(string='Appointment Date', required=False, readonly=True,states={'Scheduled': [('readonly', False)]}, default=datetime.datetime.now())
...
class OeHealthPrescriptions(models.Model):
    ...
    name = fields.Char(string='Prescription #', size=64, readonly=True, required=False, default=lambda *a: '/')
    ...
    doctor = fields.Many2one('oeh.medical.physician', string='Physician', domain=[('is_pharmacist','=',False)], help="Current primary care / family doctor", required=False, readonly=True, states={'Draft': [('readonly', False)]}, default=_get_physician)
    pharmacy = fields.Many2one('oeh.medical.health.center.pharmacy', 'Pharmacy', readonly=True, states={'Draft': [('readonly', False)]})
    date = fields.Datetime(string='Prescription Date', readonly=True, states={'Draft': [('readonly', False)]}, default=lambda self:fields.Datetime.now())
    ...
    def create(self, vals):
        ...
        vals['name'] = sequence + str(random.randint(0000,9999))
    ...
    def print_patient_prescription(self):
        return self.env.ref('oehealth.action_oeh_medical_report_patient_prescriptions_label').report_action(self)
...
class OeHealthPrescriptionLines(models.Model):
    ...
    prescription_id = fields.Many2one('oeh.medical.prescription', string='Prescription Reference', required=False, ondelete='cascade', index=True)
    name = fields.Many2one('product.product', string='Medicines', help="Prescribed Medicines", domain=[('medicament_type','=','Medicine')], required=False)
    ...
    dose_route = fields.Many2one('oeh.medical.drug.route', string='Administration Route', help="HL7 or other standard drug administration route code.", required=False)
...
class OeHealthVaccines(models.Model):
    ...
    name = fields.Many2one('product.product', string='Vaccine', domain=[('medicament_type','=','Vaccine')], required=False)
    patient = fields.Many2one('oeh.medical.patient', string='Patient', help="Patient Name", required=False)
    doctor = fields.Many2one('oeh.medical.physician', string='Physician', domain=[('is_pharmacist','=',False)], help="Current primary care / family doctor", required=False, default=_get_physician)
    date = fields.Datetime(string='Date', required=False, default=lambda self: fields.Datetime.now())
