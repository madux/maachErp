...
class OeHealthPharmacy(models.Model):
    ...
    pharmacist_name = fields.Many2one('oeh.medical.physician', string='Pharmacist Name', domain=[('is_pharmacist','=',True)], required=False)
    ...
    def _onchange_prescription_id_values(self,prescription_id):
        ...
        if vals:
            for va in vals:
                ...
                med_price = 0.0
                query1 = _("select pt.list_price from product_product po, product_template pt where po.product_tmpl_id=pt.id and po.id = %s")%(str(va[0]))
...
class OeHealthPharmacyMedicineLines(models.Model):
    ...
    price_unit = fields.Float(string='Unit Price', required=False, default=lambda *a: 0.0)
    ...
    def onchange_medicine_id(self):
        result = {}
        if self.name:
            med_obj = self.env['product.product']
