...
class PaystackController(http.Controller):
    ...
    @http.route(['/payment/paystack/verify_charge'], methods=['POST', 'GET'], type='json', auth='public')
    def paystack_verify_charge(self, **post):
        ...
        if tx.type == 'form_save' and tx.partner_id:
            ...
            response = tx._paystack_verify_charge(data)
        else:
            response = tx._paystack_verify_charge(data)
        
        _logger.info('Paystack: entering form_feedback with post data %s', pprint.pformat(response))
        
        if response:
            request.env['payment.transaction'].sudo().with_context(lang=None).form_feedback(data, 'paystackAquirer')
