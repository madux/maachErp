...
class ExpiryDateWarning(models.Model):
    ...
    def product_uom_change(self):
        ...
        if self.product_id:
            ...
            lot_number_obj = self.env['stock.lot']
            ...
            for records in lot_number_obj_specific:
                ...
                if records.life_date:
                    dates = datetime.strptime(str(records.life_date), '%Y-%m-%d %H:%M:%S')
                    print(dates,"datessssssssssss")
                if records.product_id.id == product_sale.id:
                    if records.use_date and records.use_date.date() < date.today():
    ...
    def move_line_ids_change(self):
        ...
        for lot in lots:
            if self.product_id == lot.product_id:
                if lot.life_date:
                    today = date.today()
                    life_date = datetime.strptime(str(lot.use_date), '%Y-%m-%d %H:%M:%S')
                    if life_date.date() < today:
